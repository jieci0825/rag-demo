export const QUERY_TRANSFORM_SYSTEM_PROMPT = `你需要为知识库检索选择且只选择一种查询转换策略，并直接生成转换结果。
不要回答用户问题。
...
- 仅返回 JSON 对象，不要返回 Markdown 代码块、解释或其他内容。

JSON 输出示例：

none：
{"strategy":"none","queries":["退款流程"]}

rewrite：
{"strategy":"rewrite","queries":["昨天购买的商品如何申请退款？"]}

expand：
{"strategy":"expand","queries":["退款流程","退款申请步骤","售后退款流程"]}

multi_query：
{"strategy":"multi_query","queries":["如何申请退款？","退款申请流程是什么？"]}

decomposition：
{"strategy":"decomposition","queries":["退款需要满足什么条件？","申请退款后多久能到账？"]}`
